...
class PaymentTransactionViewSet(viewsets.ModelViewSet):
    ...
    def get_object(self):
        ...
        if self.request.user.is_authenticated:
            ...
            if not guest_email or obj.guest_email != guest_email.lower():
                self.permission_denied(
                    self.request,
                    message="Invalid or missing guest email for anonymous access",
                )
        return obj
    ...
    def initiate_transaction(self, request, pk=None):
        try:
            ...
            if transaction.amount <= 0:
                logger.error(
                    f"Invalid GBP amount for {pk}: {gbp_amount_str} (currency: {transaction.currency or 'GBP'})"
                )
                return Response(
            ...
            if transaction.currency != "GBP":
                logger.warning(
                    f"Transaction {pk} currency is {transaction.currency}, but PayPal initiation requires GBP. Skipping conversion."
                )
                return Response(
                    {"error": "Transaction currency must be GBP for PayPal"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            body = {
                ...
                "purchase_units": [
                    {
                        "amount": {"currency_code": "GBP", "value": gbp_amount_str},
            ...
            logger.info(
                f"PayPal order request for tx {transaction.id}: Status {response.status_code}, Body: {response.text}"
            )
            ...
            try:
                ...
                logger.info(
                    f"PayPal order {order_data['id']} created for tx {transaction.id}"
                )
                ...
                logger.error(
                    f"PayPal order creation failed for tx {pk}: {str(e)} - Detail: {error_detail}"
                )
                return Response(
                    {
                    ...
                    status=status.HTTP_400_BAD_REQUEST,
                )  # 400 for client errors like invalid amount
    ...
    def cancel_transaction(request, tx_id):
        try:
            ...
            if transaction.booking:
                ...
                transaction.booking.save(update_fields=["status", "updated_at"])
    ...
    def capture_transaction(self, request, pk=None):
        ...
        if not order_id:
            ...
            return Response(
                {"success": False, "error": "No PayPal order ID found"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        try:
            ...
            if not captures:
                logger.error(f"No captures found in response for transaction {pk}")
                return Response(
                    {"success": False, "error": "No capture in response"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            ...
            metadata.update({"gateway": "paypal", "capture_id": capture_id})
            transaction.metadata = metadata
            transaction.status = PaymentStatus.SUCCESS
            transaction.gateway_response = dict(capture_data)
            transaction.save(update_fields=["status", "gateway_response", "metadata"])

            if transaction.booking:
                transaction.booking.status = BookingStatus.SCHEDULED
                transaction.booking.tracking_number = (
                    f"BK-{shortuuid.uuid()[:8].upper()}"
                )
                transaction.booking.save(update_fields=["status", "tracking_number"])
                logger.info(
            ...
            transaction.save(update_fields=["status", "gateway_response"])
            return Response(
                {"success": False, "error": "Capture request failed"},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
        except Exception as e:
            logger.error(f"Unexpected error capturing transaction {pk}: {str(e)}")
            return Response(
    ...
    def initiate_stripe_transaction(self, request, pk=None):
        try:
            ...
            method_type = request.data.get("method_type", "card").lower()

            type_map = {
                "card": ["card"],
                "apple_pay": ["card"],  # Apple/Google use 'card' with PaymentRequest
                "google_pay": ["card"],
                "cashapp": ["cashapp"],
                "amazon": ["amazon_pay"],
            }

            payment_method_types = type_map.get(method_type, ['card'])
...
class PaymentCallbackView(APIView):
    ...
    def _extract_order_id(self, resource):
        ...
        up_link = next((link for link in links if link.get("rel") == "up"), None)
        if up_link:
        ...
        with db_transaction.atomic():
            try:
                if event_type == "PAYMENT.CAPTURE.COMPLETED":
                    ...
                    tx.metadata.update(
                        {"gateway": "paypal", "capture_id": resource["id"]}
                    )
                    tx.save(update_fields=["status", "gateway_response", "metadata"])
                    ...
                    if not order_id:
                        ...
                        return Response(
                            {"message": "Webhook processed"}, status=status.HTTP_200_OK
                        )
                    ...
                    if tx and tx.status == PaymentStatus.PENDING:
                        ...
                        tx.gateway_response = dict(resource)
                        tx.save(update_fields=["status", "gateway_response"])
                        logger.info(f"Webhook set tx {tx.id} to FAILED (DENIED)")
                    ...
                    if tx.status not in [PaymentStatus.PENDING, PaymentStatus.SUCCESS]:
                        ...
                        return Response(
                            {"message": "Invalid status for refund"},
                            status=status.HTTP_200_OK,
                        )
                    ...
                    if not created:
                        ...
                        return Response(
                            {"message": "Duplicate refund ignored"},
                            status=status.HTTP_200_OK,
                        )
                    ...
                    return Response(
                        {"message": "Event ignored"}, status=status.HTTP_200_OK
                    )
...
class RefundViewSet(viewsets.ModelViewSet):
    queryset = Refund.objects.select_related("transaction__booking", "admin_user").all()
    ...
    def get_queryset(self):
        ...
        return Refund.objects.filter(transaction__user=user) | Refund.objects.filter(
