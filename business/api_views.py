...
class BusinessInquiryViewSet(viewsets.ModelViewSet):
    ...
    def generate_quote(self, request, pk=None):
        ...
        if inquiry.status != BusinessInquiryStatus.PENDING:
            return Response(
                {"detail": "Quote can only be generated for pending inquiries"},
                status=status.HTTP_400_BAD_REQUEST,
            )

        data = request.data or {}
        shipping_type = inquiry.shipping_type or ShippingType.objects.get(
            id=data.get("shipping_type_id")
        )
        service_type = inquiry.service_type or ServiceType.objects.get(
            id=data.get("service_type_id")
        )
    ...
    def create_booking(self, request, pk=None):
        ...
        if inquiry.status != BusinessInquiryStatus.QUOTED or not inquiry.quote:
            return Response(
                {"detail": "Booking can only be created for quoted inquiries"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        pickup_address = (
            ...
            or Address.objects.get(id=data.get("pickup_address_id"))
            if data.get("pickup_address_id")
            else None
        )
        dropoff_address = (
            inquiry.dropoff_address
            or Address.objects.get(id=data.get("dropoff_address_id"))
        ...
        booking = Booking.objects.create(
            customer=(
                inquiry.user
                if inquiry.user and inquiry.user.role == "customer"
                else None
            ),
    ...
    def approve_booking(self, request, pk=None):
        ...
        if inquiry.status != BusinessInquiryStatus.BOOKED or not inquiry.booking:
            return Response(
                {"detail": "Only booked inquiries can be approved"},
                status=status.HTTP_400_BAD_REQUEST,
            )
    ...
    def assign_driver(self, request, pk=None):
        ...
        if inquiry.status != BusinessInquiryStatus.BOOKED or not inquiry.booking:
            return Response(
                {"detail": "Only booked inquiries can have drivers assigned"},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        if not driver_id:
            return Response(
                {"detail": "driver_profile_id required"},
                status=status.HTTP_400_BAD_REQUEST,
            )
